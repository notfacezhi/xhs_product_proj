...
def crawl_content_qw(set_pause, note_id, note_url):
    ...
    # db_manager.close()
    # print(f'成功存入{len(comment_text_list)}条评论数据到数据库')
    ...
    with sync_playwright() as p:
        ...
        for i, (note_id, note_url, note_keyword) in enumerate(notes_list):
            try:
                ...
                print(f"[{note_id}] 成功存入 {len(comment_list)} 条评论到数据库")
                success_count += 1
...
if __name__=="__main__":
    ...
    # crawl_multiple_notes_from_db(max_concurrent=2, limit=10, date_filter='2026-01-06')
