...
import subprocess
import psutil
from comment_agent2 import research_demand
...
def crawl_content_qw(set_pause,keyword):
    ...
    with sync_playwright() as p:
        ...
        for i in range(note_count):
            ...
            try:
                note_href=note_i.locator('a.cover.mask.ld').get_attribute('href',timeout=15000)
            except Exception as e:
                continue
            # 从笔记的href 生成笔记id
            note_id=extract_note_id(note_href)
            #bug
            # 检查该笔记是否已被调研过（有任何调研记录就跳过）
            if db_manager.check_note_researched(note_id):
                print(f'笔记 {note_id} 已被调研过，跳过')
                continue
            print(f'笔记 {note_id} 未被调研过，开始处理')
            try:
            ...
            while comment_index < parent_comment_count and current_note_replied < max_reply_count:
                ...
                if p_comment.is_visible(timeout=15000):
                    ...
                    print('需求调研agent 开始分析')
                    agent_res=research_demand(note_title,note_desc,comment_content)
                    print(f'评论内容:{comment_content}')
                    print(f'调研结果: 新人={agent_res.is_newcomer}, 起号意图={agent_res.has_launch_intent}, 需求等级={agent_res.demand_level}')
                    ...
                    record_data = {
                        ...
                        'is_newcomer': 1 if agent_res.is_newcomer else 0,
                        'has_launch_intent': 1 if agent_res.has_launch_intent else 0,
                        'demand_level': agent_res.demand_level,
                        'reply_text': agent_res.reply_text if agent_res.reply_text else '',
                        'reason': agent_res.reason if agent_res.reason else '',
                        'is_like': 1 if (agent_res.is_newcomer and agent_res.has_launch_intent) else 0,
                        'reply_bool': 1 if agent_res.reply_text else 0,
                        'keyword': keyword
                    }
                    # 对新人+有起号意图的评论进行点赞
                    if agent_res.is_newcomer and agent_res.has_launch_intent:
                    ...
                    if agent_res.reply_text:
                        try:
                            ...
                            print('入库数据')
                            print(record_data)
                            db_manager.insert_demand_research_record(record_data)
                            print(f'已将需求调研记录存入数据库')
